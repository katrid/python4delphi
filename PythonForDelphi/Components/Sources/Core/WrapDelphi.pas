...
Type
  ...
  private
    ...
    {$IFDEF DELPHIXE_OR_HIGHER}
    function _getRttiAttr(Instance: TObject; const AttrName: string): PPyObject;
    function _setRttiAttr(const AttrName: string; Value: PPyObject): Integer;
    {$ENDIF}
  protected
...
function TPyDelphiObject._getRttiAttr(Instance: TObject; const AttrName: string)
  : PPyObject;
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        else if prop.PropertyType.TypeKind
          in [tkInt64, tkInteger, tkString, tkWString, tkWChar, tkChar, tkUString] then
          Result := GetPythonEngine.VariantAsPyObject(prop.GetValue(Instance).AsVariant)
        else if prop.PropertyType.TypeKind = tkEnumeration then
          Result := GetPythonEngine.VariantAsPyObject(prop.GetValue(Instance).AsOrdinal);
        // ShowMessage(AttrName);
      end
      else if Instance is TComponent then
      begin
        comp := TComponent(Instance).FindComponent(AttrName);
        if comp <> nil then
          Result := Wrap(comp);
      end;
    end;
...
function TPyDelphiObject._setRttiAttr(const AttrName: string;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        s := GetPythonEngine.PyObjectAsString(Value);
        prop.SetValue(DelphiObject, TValue.FromVariant(s));
      end
      else if prop.PropertyType.TypeKind = tkVariant then
      begin
        s := GetPythonEngine.PyObjectAsVariant(Value);
        prop.SetValue(DelphiObject, TValue.FromVariant(s));
      end
      else
        prop.SetValue(DelphiObject, TValue.FromVariant(GetPythonEngine.PyObjectAsVariant(Value)));
      Result := 1;
    except
      on E: Exception do
        GetPythonEngine.PyErr_SetString(GetPythonEngine.PyExc_AttributeError^,
...
{$IFDEF DELPHIXE_OR_HIGHER}
  Result := _getRttiAttr(DelphiObject, Name);
  if Result <> nil then
    Exit;
{$ENDIF}
...
{$IFDEF DELPHIXE_OR_HIGHER}
  else if _setRttiAttr(Name, Value) = 1 then
    Result := 0
{$ENDIF}
  else
...
Var
  ...
  function Wrapper: TPyDelphiWrapper;
  var
    i: Integer;
  ...
  begin
    ...
    for i := 0 to Engine.ClientCount do
      if Engine.Clients[i] is TPyDelphiWrapper then
      begin
        Result := TPyDelphiWrapper(Engine.Clients[i]);
        Break;
      end;
  end;
...
//  SetLength(ParamIndexes, 0);
  ...
  Context := TRttiContext.Create;
  meth := Context.GetType(DelphiObject.ClassInfo).GetMethod(MethName);
  ...
  begin
    arg := args[i];
    if arg.ParamType.TypeKind = tkClass then
      Params[i] := TPyDelphiObject(Integer(V[i])).DelphiObject
    else
      Params[i] := TValue.FromVariant(V[i]);
  end;
// Params[i] := TValue.FromVariant(V[i]);
  ...
  try
    try
      ...
      on E: Exception do
        PyErr_SetObject(PyExc_TypeError^, VariantAsPyObject(
          Format('"%s" called with invalid arguments: %s',
          [meth.Name, E.Message])));
    end;
  finally
    Context.Free;
